...
let DispatcherModule = function () {
  ...
  function dispatchToSubscribers(payload) {
    let subscribers = _subjectMap[payload.type], i;
    if (!subscribers) {
      return;
    }
  ...
  function unsubscribe(evtStr, handler) {
    ...
    let subscribers = _subjectMap[evtStr],
        handlerIdx  = -1;
